...
| `planted` | boolean (default `true`) | idle_cycle: solve both legs against their rest ankles so the feet stay on the floor while the pelvis moves (on; pass false for the old pelvis-only clip, e.g. when you key the feet yourself). |
